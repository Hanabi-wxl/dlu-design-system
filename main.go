...
func gormGen() {
	var dsn = "design-system:NbW6ystkMakeLzGC@(123.56.105.80:3306)/design-system?charset=utf8mb4&parseTime=True&loc=Local"
	// 连接数据库
	...
	// 模型自定义选项组
	fieldOpts := []gen.ModelOpt{jsonField, autoCreateTimeField, autoUpdateTimeField, softDeleteField, softDelField}

	// 创建模型的结构体,生成文件在 model 目录; 先创建的结果会被后面创建的覆盖
	// 这里创建个别模型仅仅是为了拿到`*generate.QueryStructMeta`类型对象用于后面的模型关联操作中
	//allModel := g.GenerateAllTable(fieldOpts...)
	//g.ApplyBasic(allModel...)
	school := g.GenerateModel("school", fieldOpts...)
